...
print(f'Bienvenue dans ce programme de vérification des constantes de votre nourrisson')

#Controles sur le genre
while genre_ok == False :
    genre = input('Veuillez entrer le genre de votre nourrisson(\'g\' pour garçon, \'f\' pour fille): ')
...
#Controles sur le poids
while poids_ok == False :
...
#Controles sur la taille
while taille_ok == False :
...
#Controles sur le périmètre cranien
while skulls_ok == False :
    skulls = input('Veuillez entrer le périmètre cranien de votre nourrisson en cm: ')
    try:
       skulls = float(skulls)
       skulls_ok = True
    except ValueError:
...
norme_poids_basse = 0
norme_poids_haute = 0
norme_taille_basse = 0
norme_taille_haute = 0
norme_skulls_basse = 0
norme_skulls_haute = 0
genre_libelle = ' '
genre_suffixe = ' '

#Traitement du poids
if genre == 'g' :
    genre_libelle = 'garçon'
    genre_suffixe = ''
    norme_poids_basse = low_weights_boys[age]
    norme_poids_haute = high_weights_boys[age]
else :
    genre_libelle = 'fille'
    genre_suffixe = 'e'
    norme_poids_basse = low_weights_girls[age]
    norme_poids_haute = high_weights_girls[age]

print(f'la norme de poids pour un{genre_suffixe} {genre_libelle} de {age} mois est situé entre {norme_poids_basse} kg et {norme_poids_haute} kg')

if poids >= norme_poids_basse and poids <= norme_poids_haute :
    print(f'le poids de votre nourrisson ({poids} kg) est dans la norme !)')
else : 
    print(f'le poids de votre nourrisson ({poids} kg) est hors norme !)')

print(f'\n')

#Traitement de la taille
if genre == 'g' : 
    norme_taille_basse = low_heights_boys[age]
    norme_taille_haute = high_heights_boys[age]
else : 
    norme_taille_basse = low_heights_girls[age]
    norme_taille_haute = high_heights_girls[age]

print(f'la norme de taille pour un{genre_suffixe} {genre_libelle} de {age} mois est situé entre {norme_taille_basse} cm et {norme_taille_haute} cm')

if taille >= norme_taille_basse and taille <= norme_taille_haute :
    print(f'la taille de votre nourrisson ({taille} cm) est dans la norme !)')
else : 
    print(f'la taille de votre nourrisson ({taille} cm) est hors norme !)')

print(f'\n')

#Traitement du périmètre cranien
if genre == 'g' :
    norme_skulls_basse = low_skulls_boys[age]
    norme_skulls_haute = high_skulls_boys[age]
else : 
    norme_skulls_basse = low_skulls_girls[age]
    norme_skulls_haute = high_skulls_girls[age]

print(f'la norme de périmètre cranien pour un{genre_suffixe} {genre_libelle} de {age} mois est situé entre {norme_skulls_basse} cm et {norme_skulls_haute} cm')

if skulls >= norme_skulls_basse and skulls <= norme_skulls_haute :
    print(f'le périmètre cranien de votre nourrisson ({skulls} cm) est dans la norme !)')
else : 
    print(f'le périmètre cranien de votre nourrisson ({skulls} cm) est hors norme !)')
